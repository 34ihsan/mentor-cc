...
import { adminUpdateUserAction } from "@/app/actions/admin-user-actions";
import { 
    User, 
    Mail, 
    Lock, 
    ...
    if (success) {
        return (
            <div className="flex flex-col items-center justify-center py-20 animate-in zoom-in duration-500">
                <div className="w-24 h-24 rounded-[40px] bg-indigo-50 text-indigo-500 flex items-center justify-center mb-8 shadow-2xl shadow-indigo-500/20">
                    <CheckCircle2 size={48} />
                </div>
                <h2 className="text-3xl font-black text-slate-900 tracking-tight mb-2">Güncelleme Başarılı!</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            ...
                            <input 
                                ...
                                className="w-full bg-slate-50 border border-slate-100 px-5 py-4 rounded-2xl text-sm font-bold focus:outline-none focus:border-[var(--primary)] focus:bg-white transition-all shadow-sm"
                            />
                        </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">ŞİFRE (BOŞ BIRAKIRSANIZ DEĞİŞMEZ)</label>
                            <div className="relative">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select 
                                ...
                                className="w-full bg-slate-50 border border-slate-100 px-5 py-4 rounded-2xl text-sm font-bold focus:outline-none focus:border-[var(--primary)] focus:bg-white transition-all shadow-sm appearance-none"
                            >
            ...
            <div className="pt-10 flex items-center justify-between border-t border-slate-100">
                ...
                <button 
                    disabled={loading}
                    type="submit"
                    className="premium-btn flex items-center gap-3 px-12 py-5 shadow-2xl shadow-indigo-900/20 bg-indigo-600 disabled:opacity-50 disabled:cursor-not-wait"
                >
                    {loading ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}
                    <span className="font-black uppercase tracking-[0.2em] text-[10px]">DEĞİŞİKLİKLERİ KAYDET</span>
                </button>
            </div>
        </form>
    );
